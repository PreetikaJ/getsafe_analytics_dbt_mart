...
with finance_transaction as (
    select
        ...
        round(sum(case when premium_status in ('processed','process', 'refunded') then premium_amount end),2) as gross_finance_premium,
...
select *, 
        case
            when gross_finance_premium < accounting_premium then 'finance_lower_than_accounting'
            when gross_finance_premium > accounting_premium then 'finance_higher_than_accounting'
            when gross_finance_premium = accounting_premium then 'finance_equal_to_accounting'
            else 'unknown'
        end as discrepancy_outcome 
from transaction_joined 
order by 1, 2